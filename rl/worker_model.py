...
sys.path.insert(0, os.path.join(
    sim_DIR, 'bullet3/build_cmake/examples/pybullet'))
...
class Worker(object):
    def __init__(self, params):
        ...
        if self.params.stage == 'imitation':
            self.file_name = '{}/{}/{}'.format("imitation", str(
                self.params.exp_name), datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        elif self.params.stage == 'test' or self.params.stage == "imitation_test":
            self.file_name = '{}_{}/{}'.format(str(self.params.task_id),
            ...
            self.file_name = '{}_{}/{}_{}_{}'.format(str(self.params.task_id), str(self.params.exp_name), str(
                self.params.method), self.params.comment, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
        elif self.params.stage == "demonstration":
            self.model_id = self.params.restore_path.strip().split('/')[-1]
            self.file_name = '{}/{}/{}'.format(str(self.params.task_id), str(
                self.params.exp_name), str(self.model_id))
            ...
            self.file_name = '{}/{}/{}'.format("imitation_feedback", str(
                self.params.exp_name), datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        else:
            self.file_name = None
        ...
        if self.params.stage == 'train' or self.params.stage == 'imitation' or self.params.stage == "feedback_train" or self.params.stage == 'imitation_feedback':
            self.writer = SummaryWriter(logdir=os.path.join(
                self.params.log_dir, self.file_name))
            ...
            self.model_dir = None
            self.writer = None
            self.saving_data_dir = os.path.join(
                self.params.demonstration_dir, self.file_name)
        ...
        if (self.params.stage == "imitation"):
            ...
            task_model = LangEncoder(device="cuda")
            task_model.eval()
            self.task_vec = task_model(task_string).squeeze().to("cpu")

            # self.task_vec = np.load(
            #     "../Languages/" + str(self.TaskId) + '.npy')
    ...
    def train(self, restore_episode=0, restore_path=None, restore_episode_goal=0, restore_path_goal=None):
        if self.params.force_term:
            self.train_force(restore_episode=restore_episode, restore_path=restore_path,
                             restore_episode_goal=restore_episode_goal, restore_path_goal=restore_path_goal)
        else:
            ...
            for ep_iter in range(1, self.params.max_ep):
                ...
                while True:
                    ...
                    if total_episode < self.params.start_learning_episode:
                        goal_pred = np.random.uniform(-0.5,
                                                      0.5, size=self.params.a_dim)
                        ...
                        goal_pred, _ = self.agent.choose_action(
                            observation, self.task_vec)
                        goal_pred = np.random.normal(goal_pred, explore_var)

                    force_pred = np.zeros(
                        (self.params.traj_timesteps, self.params.a_dim))

                    # Clip by max action
                    goal_pred[:3] = goal_pred[:3].clip(
                        -self.params.max_action, self.params.max_action)
                    goal_pred[3:6] = goal_pred[3:6].clip(
                        -self.params.rotation_max_action, self.params.rotation_max_action)

                    self.writer.add_scalar(
                        "train/explore_var", explore_var, ep_iter)

                    observation_next, reward, done, suc = self.env.step(
                        goal_pred, None, None, reset_flag)
                    ...
                    while not done:
                        ...
                        observation_next, reward, done, suc = self.env.step(
                            action_null, None, feedback_flag, reset_flag)
                        if not done:
                            reward = 0
                    ...
                    action_penalty = np.mean(
                        np.abs(goal_pred)) * self.params.action_penalty
                    action_penalty2 = np.mean(np.abs(
                        force_pred)) / float(self.params.traj_timesteps) * 0.3 * self.params.action_penalty
                    ...
                    while len(self.env.real_traj_list) < self.env.dmp.timesteps:
                        self.env.real_traj_list.append(
                            self.env.real_traj_list[-1])
                    ...
                    traj_diff = np.mean(
                        (traj_pred - traj_real) ** 2) * float(suc)
                    ...
                    print(force_pred[47])
                    self.writer.add_scalar(
                        "train_action_0/pred_action", goal_pred[0], ep_iter)
                    self.writer.add_scalar(
                        "train_action_1/pred_action", goal_pred[1], ep_iter)
                    self.writer.add_scalar(
                        "train_action_2/pred_action", goal_pred[2], ep_iter)
                    self.writer.add_scalar(
                        "train_action_6/pred_action", goal_pred[6], ep_iter)
                    self.writer.add_scalar(
                        "train_reward/action_penalty", action_penalty, ep_iter)
                    self.writer.add_scalar(
                        "train_reward/action_penalty2", action_penalty2, ep_iter)
                    self.writer.add_scalar(
                        "train_reward/traj_diff_penalty", traj_diff_penalty, ep_iter)
                    ...
                    action_pred = np.zeros(
                        ((self.params.traj_timesteps + 1) * self.params.a_dim,))
                    action_pred[:self.params.a_dim] = goal_pred
                    action_pred[self.params.a_dim:] = force_pred.reshape((-1,))

                    self.agent.store_transition(
                        initial_pose, traj_real, observation, action_pred, reward, done, self.task_vec, goal_pred)
                    ...
                    if ep_iter % self.params.saving_model_freq == 0:
                        current_performance = float(
                            suc_num) / (episode_num + 0.0001)
                        print("suc rate %f reward %f var %f" % (current_performance, float(
                            reward_check) / (episode_num + 0.0001), explore_var))
                        ...
                        self.writer.add_scalar(
                            "test/success_rate", cur_perf, ep_iter)

                    if done:
                        self.writer.add_scalar(
                            "train/success_rate", suc, ep_iter)
                        suc_num += suc
                        episode_num += 1
                        break
    ...
    def train_force(self, restore_episode=0, restore_path=None, restore_episode_goal=0, restore_path_goal=None):
        ...
        if restore_path is not None and restore_episode > 0:
            print("restore_episode_goal", restore_episode_goal)
            self.agent.restore_actor_goal_only(
                step=restore_episode_goal, restore_path=restore_path_goal)
            self.agent.restore_actor(
                step=restore_episode_goal, restore_path=restore_path_goal)
        ...
        for ep_iter in range(1, self.params.max_ep):
            ...
            while True:
                ...
                if total_episode < self.params.start_learning_episode:
                    ...
                    force_pred = np.random.uniform(-1.0, 1.0, size=(
                        self.params.traj_timesteps, self.params.a_dim))
                    weights = np.linspace(1, 0, self.params.traj_timesteps).reshape(
                        (self.params.traj_timesteps, 1)) * float(self.params.traj_timesteps)
                    force_pred = force_pred * weights
                else:
                    explore_var = max(explore_var, 0.1)
                    goal_pred_goal_only = self.agent.choose_action_goal_only(
                        observation, self.task_vec)
                    goal_pred, force_pred = self.agent.choose_action(
                        observation, self.task_vec)
                    scales = np.linspace(1, 0, self.params.traj_timesteps).reshape(
                        (self.params.traj_timesteps, 1)) * float(self.params.traj_timesteps) * explore_var
                    scales = np.tile(scales, [1, self.params.a_dim])
                    action_max = np.linspace(1, 0, self.params.traj_timesteps).reshape(
                        (self.params.traj_timesteps, 1)) * float(self.params.traj_timesteps)
                ...
                observation_next, reward, done, suc = self.env.step(
                    goal_pred, force_pred, None, reset_flag)
                ...
                action_penalty = np.mean(
                    np.abs(force_pred)) / float(self.params.traj_timesteps) * 0.1
                ...
                self.writer.add_scalar(
                    "train_action_6/pred_action", goal_pred[6], ep_iter)
                self.writer.add_scalar(
                    "train_action_0/gt_action", goal_pred_goal_only[0], ep_iter)
                self.writer.add_scalar(
                    "train_action_1/gt_action", goal_pred_goal_only[1], ep_iter)
                self.writer.add_scalar(
                    "train_action_2/gt_action", goal_pred_goal_only[2], ep_iter)
                self.writer.add_scalar(
                    "train_action_6/gt_action", goal_pred_goal_only[6], ep_iter)
                self.writer.add_scalar(
                    "train_action_0/pred_force", np.mean(force_pred[0]), ep_iter)
                self.writer.add_scalar(
                    "train_action_1/pred_force", np.mean(force_pred[1]), ep_iter)
                self.writer.add_scalar(
                    "train_action_2/pred_force", np.mean(force_pred[2]), ep_iter)
                self.writer.add_scalar(
                    "train_action_6/pred_force", np.mean(force_pred[6]), ep_iter)
                self.writer.add_scalar(
                    "train_reward/action_penalty", action_penalty, ep_iter)
                self.writer.add_scalar(
                    "train_reward/traj_diff_penalty", traj_diff_penalty, ep_iter)
                ...
                self.agent.store_transition(
                    initial_pose, traj_real, observation, action_pred, reward, done, self.task_vec, action_gt)
                ...
                if ep_iter % self.params.saving_model_freq == 0:
                    ...
                    self.agent.save_model_actor_critic(ep_iter)
                    cur_perf = self.test(
                        ep_iter, restore_episode_goal=restore_episode_goal, restore_path_goal=restore_path_goal)
                    self.writer.add_scalar(
                        "test/success_rate", cur_perf, ep_iter)
    ...
    def test(self, restore_episode=0, restore_path=None, restore_episode_goal=0, restore_path_goal=None):
        ...
        for ep_iter in range(max_iteration):
            ...
            while True:
                # First generate action
                if self.params.stage == "imitation" or self.params.stage == "imitation_test":
                    goal_pred, force_pred = self.agent.choose_action_master(
                        observation, self.task_vec)
                else:
                    if self.params.force_term:
                        goal_pred, force_pred = self.agent.choose_action(
                ...
                while not done:
                    ...
                    observation_next, reward, done, suc = self.env.step(
                        np.zeros((7,)), None, feedback_term, reset_flag)
                print(force_pred[0])
                print(force_pred[1])
                print(force_pred[2])
                print(force_pred[20])
                ...
                print(force_pred[40])
                print(force_pred[47])
                print("Testing", "success", suc, "taskid", self.TaskId, "ep_iter", ep_iter, " goal_pred", goal_pred, "force_pred", force_pred[0],
                ...
                if done:
                    ...
                    if recordGif:
                        classes = [line.strip().split(":")[0]
                                   for line in open('../Languages/labels.txt')]
                        recordGif_dir = os.path.join(
                            self.params.gif_dir, str(self.params.task_id))
        ...
        if (not self.params.stage == 'imitation') or (not self.params.stage == 'imitation_test'):
            np.savetxt(os.path.join(self.log_dir, 'successRate.txt'),
                       np.array([perf]), fmt='%1.4f')
    ...
    def imitate(self):
        ...
        meta_info = [line.strip().split()
                     for line in open("pretrained_model_list.txt")]
        save_top_dir_dict = {}
        save_model_dir = {}
        len_example_list = {}
        perf_dict = {}
        for meta_i in meta_info:
            ...
            len_example_list[meta_i[0]] = int(
                len([line for line in os.listdir(meta_i[4])]) * 0.99)
        ...
        if 0:  # self.params.task_id > 0:
            ...
            # [45,43,600,604,608,609,610,611,86]#
        ...
        np.savetxt(os.path.join(self.log_dir, 'successRate_cur.txt'),
                   task_perf_cur, fmt='%1.4f')
        np.savetxt(os.path.join(self.log_dir, 'successRate_upbound.txt'),
                   task_perf_upbound, fmt='%1.4f')
        np.savetxt(os.path.join(self.log_dir, 'progress.txt'),
                   progress, fmt='%1.4f')
        ...
        for b_task in task_id_list:
            ...
            example_name_dict[str(b_task)] = [line for line in os.listdir(
                save_sub_dir) if int(line) < len_example_list[b_task]]
        ...
        for i_iter in range(0, self.params.max_iteration):
            ...
            for task_id in task_id_list:
                bs_dict[task_id] = 0
            task_index_bs = np.random.choice(
                task_id_list, self.params.batch_size, p=prob)
            ...
            for task_id, progress_id, prob_id, error_id in zip(task_id_list, progress, prob, error_list):
                print_string.append(
                    (task_id, progress_id, prob_id, bs_dict[task_id], error_id))
            print(print_string)
            task_example_list = []
            for task_index in task_index_bs:
                task_index_bs1 = np.random.randint(
                    1, example_num_dict[str(task_index)])
            ...
            if i_iter % self.params.saving_model_freq_imitation == self.params.saving_model_freq_imitation - 1:
                ...
                for task_id in task_id_list:
                    ...
                    imitate_memory = np.array(task_example_list)
                    eval_goal_loss, eval_force_loss, eval_loss = self.agent.imitate_learn_test(
                        imitate_memory)
...
#                        self.params.restore_old = True
                    # restore_episode=save_model_dir[self.TaskId][1], restore_path=save_model_dir[self.TaskId][0])
                    perf = self.test()
                    # error = 0.001
                    # error_list[idx] = error
                    task_perf_cur[idx] = perf
                for (task_id, eval_err, test_err) in zip(task_id_list, eval_error_list, error_list):
                    print("task_id", task_id, "eval_err",
                          eval_err, "test_error", test_err)
                # self.params.writer.add_scalar('test_perf',perf,i_iter)
                np.savetxt(os.path.join(self.log_dir, 'successRate_cur_' +
                           str(i_iter)+'.txt'), task_perf_cur, fmt='%1.4f')
                np.savetxt(os.path.join(
                    self.log_dir, 'successRate_upbound.txt'), task_perf_upbound, fmt='%1.4f')
                progress = task_perf_cur / task_perf_upbound
                np.savetxt(os.path.join(self.log_dir, 'progress_' +
                           str(i_iter)+'.txt'), progress, fmt='%1.4f')
                self.agent.master.eval()
